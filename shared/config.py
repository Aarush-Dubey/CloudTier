...
class Settings:
    ...
    leader_lock_collection: str = os.getenv("LEADER_LOCK_COLLECTION", "leader_locks")
    ...
    job_lock_ttl_sec: float = _float("JOB_LOCK_TTL_SEC", 30.0)
    leader_lease_ttl_sec: float = _float("LEADER_LEASE_TTL_SEC", 10.0)
    leader_heartbeat_sec: float = _float("LEADER_HEARTBEAT_SEC", 3.0)
    optimizer_scan_interval_sec: float = _float("OPTIMIZER_SCAN_INTERVAL_SEC", 30.0)
    instance_id: str = os.getenv("INSTANCE_ID", os.getenv("HOSTNAME", ""))
